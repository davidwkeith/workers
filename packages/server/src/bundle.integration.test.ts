 ...
 * cleanly on SIGTERM.
 *
 * Skipped unless this package and `@dwk/cf-shims` are built (the bundle
 * aliases both packages' `dist/`), which CI does before the test step.
 *
...
describe.skipIf(!existsSync(distEntry) || !existsSync(distShim))(
  "self-host esbuild bundle",
  () => {
