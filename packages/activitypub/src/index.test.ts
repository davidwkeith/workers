...
describe("publish endpoint", () => {
  ...
  it("400s an unrecognized skipDelivery value instead of silently publishing live", async () => {
    const config = makeConfig({ publishToken: "s3cret" });
    const handler = createActivityPub(config);
    const res = await handler(
      new Request(`${actorUrl(config)}/outbox?skipDelivery=true`, {
        method: "POST",
        headers: {
          authorization: "Bearer s3cret",
          "content-type": "application/activity+json",
  ...
  it("skipDelivery on /publish inserts a shaped post without queuing follower delivery", async () => {
    const config = makeConfig({ publishToken: "s3cret" });
    const handler = createActivityPub(config);
    const iris = deriveIris(config.baseUrl, config.actor.username);
    const stub = testEnv.ACTOR.get(testEnv.ACTOR.idFromName(iris.id));
    ...
    const created = await handler(
      new Request(`${actorUrl(config)}/publish?skipDelivery=1`, {
        method: "POST",
        headers: {
          authorization: "Bearer s3cret",
