...
- **Exact legacy `rel` matching (sender discovery).** Endpoint discovery matched
  the legacy rel with `startsWith("http://webmention.org")`, which also accepted
  look-alike hosts like `http://webmention.org.evil.example/`. It now matches the
  exact legacy strings `http://webmention.org/` and
